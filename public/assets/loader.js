...
// Convert CIDv0 to CIDv1 util
const base58Alphabet="123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz",base32Alphabet="abcdefghijklmnopqrstuvwxyz234567";function decodeBase58(e){let t=0n;for(let n of e)t=58n*t+BigInt("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz".indexOf(n));let o=t.toString(16);return o.length%2&&(o="0"+o),Uint8Array.from(o.match(/.{1,2}/g).map(e=>parseInt(e,16)))}function encodeBase32(e){let t="",n="";for(let o of e)t+=o.toString(2).padStart(8,"0");for(let a=0;a<t.length;a+=5)n+="abcdefghijklmnopqrstuvwxyz234567"[parseInt(t.slice(a,a+5).padEnd(5,"0"),2)];return n}function cidV0ToV1(e){let t=decodeBase58(e);if(34!==t.length||18!==t[0]||32!==t[1])throw Error("Invalid CIDv0");let n=new Uint8Array([1,112,...t]);return"b"+encodeBase32(n)}

const invokeLoader = (function () {
    const opts = {};
    let progressPanel = null;
    let progressStr1 = null;
    let progressStr2 = null;
    let progressStr3 = null;
    ...
    function makePatternA(domain) {
        return (cid, path) => { return "https://" + domain + "/ipfs/" + cid + "/" + path; };
    }

    function makePatternB(domain) {
        return (cid, path) => { return "https://" + cidV0ToV1(cid) + ".ipfs." + domain + "/" + path; };
    }

    const IPFS_GATEWAYS = [
        makePatternA("gateway.pinata.cloud"),
        makePatternA("ipfs.io"),
        makePatternA("gateway.ipfs.io"),
        makePatternB("4everland.io"),
        makePatternA("dweb.link"),
        ...
        makePatternA("ipfs.oceanprotocol.com"),
        makePatternB("flk-ipfs.xyz")
    ];
    ...
    function loadClientFile(arrayBuffer) {
        ...
        if(opts.launchCallback !== undefined) {
            opts.launchCallback(arrayBuffer)
        } else {
    ...
    function saveClientToIndexedDB(fileData) {
        return new Promise((resolve) => {
            ...
            openRequest.addEventListener("success", () => {
                ...
                writeRequest.addEventListener("success", () => {
                    resolve(true);
                });
                transaction.addEventListener("success", () => {
                    db.close();
    ...
    (async function () {
        ...
        opts.launchCallback = window.__eaglercraftLoaderClient.launchCallback;
...
async function fetchIPFS(version, type, callback = (loaderData) => {}) {
    try {
        let response = await fetch("../api/ipfs");
        if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
        let data = await response.json();

        let versionMetadata = data[version][type];
        ...
        window.__eaglercraftLoaderClient = {
            ...
            cid: versionMetadata.cid,
            path: "",
            dlSize: versionMetadata.size,
            gzip: true
        };

        callback(__eaglercraftLoaderClient)

        invokeLoader()
    } catch (error) {
        console.error("Fetch error:", error);
        alert("There was an error while fetching metadata.")
    }
}