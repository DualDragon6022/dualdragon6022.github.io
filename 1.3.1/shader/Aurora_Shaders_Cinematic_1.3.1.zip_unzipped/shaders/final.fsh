...
float lum=luma(col);if(lum>0.72){vec3 b=vec3(0.0);float w=0.0;for(int x=-BLOOM_QUALITY;x<=BLOOM_QUALITY;x++)for(int y=-BLOOM_QUALITY;y<=BLOOM_QUALITY;y++){vec3 s=texture(colortex0,uv+vec2(x,y)*px*2.0).rgb;float q=max(luma(s)-0.68,0.0);b+=s*q;w+=q;}if(w>0.001)col+=b/w*(0.018+0.014*float(BLOOM_QUALITY));}
#endif
#if STYLE==0
col=col/(col+0.155)*1.019;col=pow(max(col,0.0),vec3(0.4545));
#else
vec3 x=max(vec3(0.0),col-0.004);col=(x*(6.2*x+0.5))/(x*(6.2*x+1.7)+0.06);
#endif
outC=vec4(col,1.0);}
